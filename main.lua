...
function love.draw()
    ...
    if DEBUG then
        love.graphics.push()
        local x, y = CONFIG.debug.stats.position.x,
                     CONFIG.debug.stats.position.y
        ...
        local info = {
            ...
            "Garbage: " .. string.format("%7.2f", Lume.round(ram, .01)) ..
                memoryUnit,
            "VRAM: " .. string.format("%6.2f", Lume.round(vram, .01)) ..
                memoryUnit, "Draw calls: " .. stats.drawcalls,
            "Images: " .. stats.images, "Canvases: " .. stats.canvases,
            "Switches: " .. stats.canvasswitches,
            "Shader switches: " .. stats.shaderswitches,
            "Fonts: " .. stats.fonts
        }
        love.graphics.setFont(CONFIG.debug.stats.font[CONFIG.debug.stats
                                  .fontSize])
        for i, text in ipairs(info) do
            local sx, sy = CONFIG.debug.stats.shadowOffset.x,
                           CONFIG.debug.stats.shadowOffset.y
...
function love.errorhandler(msg)
    ...
    local max_text_width = love.graphics.getWidth() -
                               love.window
                                   .toPixels(CONFIG.debug.error.position.x)
    ...
    local function draw()
        local x, y = love.window.toPixels(CONFIG.debug.error.position.x),
                     love.window.toPixels(CONFIG.debug.error.position.y)
        love.graphics.clear(CONFIG.debug.error.background)
        local sx, sy = CONFIG.debug.error.shadowOffset.x,
                       CONFIG.debug.error.shadowOffset.y
        ...
        if math.abs(translate_vy) > 4 then
            ...
            local bar_y = -translate_y / max_translate_y *
                              love.graphics.getHeight()
            love.graphics.setColor(1, 1, 1, 0.5)
            love.graphics.rectangle('fill',
                                    love.graphics.getWidth() - bar_width - 1,
                                    bar_y, bar_width, bar_height, 2, 2)
            love.graphics.pop()
        else
            love.graphics.push()
            love.graphics.translate(0, -translate_y)
            ...
            local bar_y = -translate_y / max_translate_y *
                              love.graphics.getHeight()
            love.graphics.setColor(1, 1, 1, Lume.clamp(
                                       math.abs(translate_vy) / 4, 0, 0.5))
            love.graphics.rectangle('fill',
                                    love.graphics.getWidth() - bar_width - 1,
                                    bar_y, bar_width, bar_height, 2, 2)
            love.graphics.pop()
    ...
    return function()
        ...
        for e, a, b, c, d in love.event.poll() do
            ...
            elseif e == "wheelmoved" then
                translate_ay = b * 2000 *
                                   math.sqrt((1 + math.abs(translate_vy)))
            end
        end
        ...
        if max_translate_y > love.graphics.getHeight() then
            ...
            if translate_y > 0 then translate_y = 0 end
