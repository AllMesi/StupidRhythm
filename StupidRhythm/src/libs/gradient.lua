    ...
    for x = 0, img:getWidth() - 1 do
        for y = 0, img:getHeight() - 1 do
            ...
            elseif v == "radial" then
                local d = math.sqrt((x - img:getWidth() / 2) ^ 2 +
                                        (y - img:getHeight() / 2) ^ 2)
                a = 1 - math.min(1, d / (img:getWidth() / 2))
            elseif v == "angle" then
                local angle = -math.atan2(y - img:getHeight() / 2,
                                          x - img:getWidth() / 2) %
                                  (math.pi * 2)
                a = angle / (math.pi * 2)
            elseif v == "rhombus" then
                ...
                a = math.min(1, px + py - img:getWidth() / 2) /
                        (img:getWidth() / 2)
...
function love.gradient.draw(drawFunc, gradientType, centerX, centerY,
                            radialWidth, radialHeight, color1, color2, angle,
                            scaleX, scaleY)
    angle = angle or 0
    scaleX = scaleX or 1
    scaleY = scaleY or 1
    ...
    assert(type(gradientType) == "string",
           "Gradient's argument #1 must be a gradient type (" ..
               table.concat(love.gradient.types, ", ") .. ").")
    ...
    assert(containsGradientType,
           "Gradient's argument #2 must be a gradient type (" ..
               table.concat(love.gradient.types, ", ") .. ").")
    ...
    love.graphics.draw(gradientImg, -radialWidth, -radialHeight, 0,
                       radialWidth * 2 * scaleX / gradientImg:getWidth(),
                       radialHeight * 2 * scaleY / gradientImg:getHeight())
