...
local absindex = function(len, i) return i < 0 and (len + i + 1) or i end
...
function lume.clamp(x, min, max) return
    x < min and min or (x > max and max or x) end
...
function lume.lerp(a, b, amount) return a + (b - a) * lume.clamp(amount, 0, 1) end
...
function lume.pingpong(x) return 1 - math_abs(1 - x % 2) end
...
function lume.angle(x1, y1, x2, y2) return math_atan2(y2 - y1, x2 - x1) end
...
function lume.randomchoice(t) return t[math.random(#t)] end
...
function lume.combine(...)
    ...
    return function(...) for _, f in ipairs(funcs) do f(...) end end
end

function lume.call(fn, ...) if fn then return fn(...) end end
...
serialize = function(x, stk) return serialize_map[type(x)](x, stk) end

function lume.serialize(x) return serialize(x) end

function lume.deserialize(str) return lume.dostring("return " .. str) end
...
function lume.dostring(str) return assert((loadstring or load)(str))() end
...
function lume.hotswap(modname)
    ...
    local function update(old, new)
        ...
        for k, v in pairs(new) do
            print(string.format("%20s", k), string.format("%20s", v),
                  string.format("%10s", old[k] == nil))
...
function lume.ripairs(t) return ripairs_iter, t, (#t + 1) end
...
function lume.chain(value) return setmetatable({_value = value}, chain_mt) end

setmetatable(lume, {__call = function(_, ...) return lume.chain(...) end})
