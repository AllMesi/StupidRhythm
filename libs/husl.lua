...
function husl.hex_to_husl(hex) return husl.rgb_to_husl(husl.hex_to_rgb(hex)) end
...
function husl.hex_to_huslp(hex) return husl.rgb_to_huslp(husl.hex_to_rgb(hex)) end
...
function husl.max_chroma(L, H)
    ...
    for _, row in ipairs(m) do
        ...
        if C0 > 0 and C0 < result then result = C0 end

        if C1 > 0 and C1 < result then result = C1 end
    end
...
local function hrad_extremum(L)
    local lhs = (math.pow(L, 3) + 48 * math.pow(L, 2) + 768 * L + 4096) /
                    1560896
    local rhs = epsilon
    ...
    for _, row in ipairs(m) do
        ...
        for limit = 0, 1 do
            ...
            if limit == 1 then hrad = hrad + math.pi end
...
function husl.rgb_prepare(r, g, b)
    ...
    for i, component in ipairs {r, g, b} do
        ...
        assert(component >= -0.0001 and component <= 1.0001,
               "illegal rgb value " .. component)
...
function husl.rgb_to_xyz(r, g, b)
    local rgb = {husl.to_linear(r), husl.to_linear(g), husl.to_linear(b)}

    local xyz = {}

    for i, row in ipairs(m_inv) do xyz[i] = dot_product(row, rgb) end

    return unpack(xyz)
end

function husl.xyz_to_luv(X, Y, Z)
    if X == 0 and Y == 0 and Z == 0 then return 0, 0, 0 end
    ...
    if L == 0.0 then return 0, 0, 0 end
